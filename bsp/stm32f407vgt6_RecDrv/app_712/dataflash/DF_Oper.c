...
{   
	u16 i=0;
      ...
      BaseAddress=page_counter*PageSIZE;    
	
	for(i=0;i<length;i++)
	{
		 *p=SST25V_ByteRead(((u32)BaseAddress)+(u32)(page_offset+i));//512bytes 一个单位
		 p++;
	}	 
    //DF_delay_us(200);   
      
    //SST25V_BufferRead(p,BaseAddress,length);  	 
    DF_delay_ms(1);  
}
...
void DF_WriteFlashDirect(u16 page_counter,u16 page_offset,u8 *p,u16 length)//512bytes 直接存储
{
	...
	for(i=0;i<length;i++)
	{
		SST25V_ByteWrite(*p,(u32)BaseAddress+(u32)(page_offset+i));
		p++;
	}
       
	//SST25V_strWrite(p,BaseAddress,length);    
	DF_delay_ms(5);  
}
