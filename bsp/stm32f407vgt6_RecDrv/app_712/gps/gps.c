			...
			switch(CommaCount){
				case 2: //时间 
				   // systemTickGPS_Set();  
				    if ( iCount < 6 ) 	  //  格式检查
					  {   
					    //秒为0时数据出错统计分钟平均速度会出错，所以将其清0
					     PerMinSpdTotal=0;  // clear   
					     AspdCounter=0;	   // clear  
						
					     StatusReg_GPS_V(); 
						 rt_kprintf("\r\nERROR-time(1),iCount=%d,%s",iCount,tmpinfo);
					     return false;
					  }
					...
					//秒为0时数据出错统计分钟平均速度会出错，所以将其清0
        				     PerMinSpdTotal=0;  // clear   
        				     AspdCounter=0;	   // clear  
					    rt_kprintf("\r\nERROR-time(2),  %2x  %2x  %2x  %2x  %2x",tmpinfo[0],tmpinfo[1],tmpinfo[2],tmpinfo[3],tmpinfo[4]);
					StatusReg_GPS_V();   	
					return false;
					}
   ...
   if (strlen(str)==0)
	...
	else 
	{
		if(str[0]=='1')
			...
			{
			   dev_gps_write( &dev_gps, 0, GPS_MODE, strlen(GPS_MODE));
			   GpsStatus.Position_Moule_Status=2;
			   Car_Status[1]&=~0x0C; // clear bit3 bit2      1100
			   Car_Status[1]|=0x04; // Gps mode   0100
			//GPS_PutStr(GPS_MODE);
			...
			{
			   dev_gps_write( &dev_gps, 0, GPSBD_MODE, strlen(GPSBD_MODE));    
			   GpsStatus.Position_Moule_Status=3;
			   Car_Status[1]&=~0x0C; // clear bit3 bit2 
			   Car_Status[1]|=0x0C; // BD+GPS  mode	1100	 
			   rt_kprintf("\r\n   GPS&BD  MODEr\n");
