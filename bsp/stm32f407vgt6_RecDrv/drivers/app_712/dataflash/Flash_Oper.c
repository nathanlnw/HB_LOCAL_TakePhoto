...
//-------------------------------
  ...
  */
   //static u8   reg_save[2]={0,0};
   //static u32  pageoffset=0;   //Page 偏移
   //static u32  InPageoffset;   //页内Record偏移
   //static  u16  InPageAddr=0;   //页内 地址偏移 

u8  SaveCycleSub_1_RdSaveOperate(u16 page_counter,u16 page_offset,u8 *p,u16 length)
{
        u8   reg_save[2]={0,0};
	 u16  start=0, addr=0;	
	  ...
	  addr=page_offset;	 
       ...
       //   DF_delay_ms(1); 
        //  DF_ReadFlash(start,addr,reg_save,2); 
	  // DF_delay_ms(5); 	  
	  // if((reg_save[0]==0xff) &&(reg_save[1]==0xff)) // 如果要写入的区域 dirty  ，则地址增然后从新开始寻找直到找到为止
	   	{
	   	    	     Current_State=0;   //clear state   
			    // cycle_writeAbnormal_counter=0;  // normal sate  clear
			     DF_delay_ms(10); 
			     DF_WriteFlashDirect(page_counter,page_offset,p,length);  //   写入信息 
			     DF_delay_ms(10);  
			     return true;	    
	   	}
	 /*  else
	    {	
	          rt_kprintf("\r\n save  false   disp error area  ------> \r\n");  
	          SaveCycleSub_2_FCS_read_judge(start,addr); 
	          return false;
	    }*/

}

u8  SaveCycleSub_2_FCS_read_judge(u16 page_counter,u16 page_offset)
{
     u8   reg[32];
     u8    FCS=0;
     u16  i=0;	 
	 ...
	 DF_delay_ms(10);  	
        DF_ReadFlash(page_counter,page_offset,reg,31); 	

	 if((reg[0]==0xFF)&&(reg[1]==0xff))
	 {
	          rt_kprintf("\r\n Read  all FF \r\n");
                 return false;
	 }	  	
...
u8 SaveCycleGPS(u32 cyclewr,u8 *content ,u16 saveLen) 
{
  ...
  //	   2. Filter write  area  
          WatchDog_Feed();   
         if(SaveCycleSub_1_RdSaveOperate(pageoffset+CycleStart_offset,InPageAddr,content,saveLen)==true )
          	{
          	       WatchDog_Feed();
                   if(SaveCycleSub_2_FCS_read_judge(pageoffset+CycleStart_offset,InPageAddr)==false)
				{
				      rt_kprintf("\r\n save  1   false \r\n");
				      cycle_write++;  	
					   ...
					   cycle_writeAbnormal_counter++;
					    if(cycle_writeAbnormal_counter>3)
							...
							DF_delay_ms(50); 
							WatchDog_Feed(); 
						       rt_kprintf("\r\n 1st write error exceed 3 ,Erase current Cycle Sector : %d\r\n",(pageoffset>>3));    
							//----------------------------   
...
//RD_CYC:
  ...
  //  3. Judge FCS	
	//--------------- 过滤已经发送过的信息 -------
	  ...
	  if(((content[ReadLen-1]!=FCS)&&(content[0]!=0xFF)&&((content[1]!=0xFF)))||(content[0]==0xFF))  // 判断异或和   
	    { 	      
		  if(content[0]==0xFF)
		  {
