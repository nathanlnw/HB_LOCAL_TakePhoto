...
//----- GPS 发送本地定时 -------------
u8  local_trig=1;  //  本地定时上报
u32 local_timer=0; // 本地定时计数器
u32 Gpstimer_abnormal_counter=0; // GPS 时钟触发异常次数 
...
static u32      Bak_timer_seconds=0,Bak_delta_timer_seconds=0;
u8	        UDP_dataPacket_flag= 0x03;			   /*V	   0X03 	 ;		   A	  0X02*/
u8	        GPS_getfirst=0; 		 //  首次有经纬度 
u8            GPS_datetimeFirstGet=0;//  首次获取日期和时间
...
// 	  u8 i=0;
	    ...
	    //------------------------------------------------------------------
	     ...
	     if((1==f_ISP_23_ACK)&&(1==TCP2_Connect)) 
			...
			// {	
		      if(PositionSD_Status()&&(DEV_Login.Operate_enable==2)&&((enable==BD_EXT.Trans_GNSS_Flag)||(DispContent==6))||(Current_UDP_sd&&PositionSD_Status())||((DF_LOCK==enable)&&PositionSD_Status()))	  //首次定位再发 
			   //  if((PositionSD_Status())&&(DataLink_Status())&&(DEV_Login.Operate_enable==2))	                                                                                                                     // DF  锁定发送当前位置信息  
		      {
				  ...
				  // 2. 			  
				   ...
				   if(ReadFail_sd_current)
				   	{    
				   	      rt_kprintf("\r\n read fail send !\r\n");    
				             ReadFail_sd_current=0;
				   	}
				   if(DispContent)	
					    rt_kprintf("\r\n 发送 GPS -current !\r\n");     
			    }   
	 ...
	 if(UDP_dataPacket_flag==0x02 )
	 {
		 ...
		 // --------  sp 当前是0.1 knot------------------	 
		  ...
		  sp_DISP=sp/100;   //  sp_Disp 单位是 0.1km/h 
	         ...
	         //------------------------------ 通过GPS模块数据获取到的速度 --------------------------------
	        if(Satelite_num>=5)   // 北斗模块漂移，不超过5颗星不给速度
 ...
 if(GPRMC.status=='A')     //  记录定位时间
{
  ...
  //---- 存储当前的起始里程  跨天时------------
   if((hour==0)&&(min==0)&&(sec==8))   // 存储3次确保存储成功   
  	{ 
...
if(year&&mon&&day)
	 GPS_datetimeFirstGet=1;   // 年月日都不能为0   
		...
		else 
		 {
			 // systemTickGPS_Set();  
			  ...
			  StatusReg_GPS_V();
               ...
               //------------------
               if ( tmp_time_secnonds > Bak_timer_seconds)
		{
				Bak_delta_timer_seconds= tmp_time_secnonds - Bak_timer_seconds;
		}
		else if(tmp_time_secnonds < Bak_timer_seconds)
		{
				Bak_delta_timer_seconds=86400-Bak_timer_seconds + tmp_time_secnonds;
		} 
		...
		if((SleepState==1)&&(delta_time_seconds==(Current_SD_Duration-5)&&(Current_SD_Duration>5)))  //  --  休眠时 先发鉴权
		   {
			  SleepConfigFlag=1;  //发送前5 发送一包鉴权
               ...
               //------ Send Trigger  ---------------
             if(Current_SD_Duration==30)
             {
	               if((delta_time_seconds)&&((CurrentTime[2]==50)||(CurrentTime[2]==20)))  
	               {    //  GPS trigger
	                    //-----------------------------------------------------
	                   if(GPS_getfirst==1)
	                   	{
		                    local_trig=0; 
				      Gpstimer_abnormal_counter=0;	
				      local_timer=0;	  
	                   	}	  
	                    //------------------------------------------------------
			      PositionSD_Enable();    
		...
		if((delta_time_seconds >= Current_SD_Duration))//limitSend_idle
		  {
			  ...
			  memcpy(BakTime,CurrentTime,3); // update    
                ...
                //-----  Enable Local Timer ---------
                	 if(Bak_delta_timer_seconds==0)	
			 {  //  Enable local timer                      		
				Gpstimer_abnormal_counter++;  
				if(Gpstimer_abnormal_counter==20)
				{
				       Gpstimer_abnormal_counter=0;
					if(local_trig==0)
						   rt_kprintf("\r\n-->Enable  local time !\r\n");
                ...
                Bak_timer_seconds=tmp_time_secnonds;

	   
  	}  
  
    //------------------------------ do this every  second-----------------------------------------  
    lat=(Temp_Gps_Gprs.Latitude[0]<<24)+(Temp_Gps_Gprs.Latitude[1]<<16)+(Temp_Gps_Gprs.Latitude[2]<<8)+Temp_Gps_Gprs.Latitude[3];
    longit=(Temp_Gps_Gprs.Longitude[0]<<24)+(Temp_Gps_Gprs.Longitude[1]<<16)+(Temp_Gps_Gprs.Longitude[2]<<8)+Temp_Gps_Gprs.Longitude[3];
    if(lat&&longit)
...
{ 
      	if((local_trig==1)||(GPS_getfirst==2)) // 	GPS 时间
	{
         if(Current_SD_Duration==30)
         {
               // date
               Gps_Gprs.Date[0]=time_now.year;
		 Gps_Gprs.Date[1]=time_now.month;
		 Gps_Gprs.Date[2]=time_now.day;	  	
		 //  time	 
               Gps_Gprs.Time[0]=time_now.hour;
		 Gps_Gprs.Time[1]=time_now.min;
		 Gps_Gprs.Time[2]=time_now.sec;

               if((Gps_Gprs.Time[2]==20)&&(Gps_Gprs.Time[2]==50))
               {
                     PositionSD_Enable();   
			 Current_UDP_sd=1; 	 		 
			rt_kprintf("\r\n-->local time trig-30\r\n"); 		 
          ...
          {
             local_timer++;	  
		if((SleepState==1)&&(local_timer==(Current_SD_Duration-5))&&(Current_SD_Duration>5))  //  --  休眠时 先发鉴权
		   {
			  SleepConfigFlag=1;  //发送前5 发送一包鉴权
		   }            
		
		if((local_timer >= Current_SD_Duration))//limitSend_idle
		  {
			  PositionSD_Enable();    
			  Current_UDP_sd=1; 	 
			  local_timer=0;// clear
...
{    //  GPS 不定位      河北版本就填写 V 情况
  //  if(GPS_getfirst==1)
      // Car_Status[3]|=0x02;   //满足河北对数据有效性的要求  首次定位后，不定位的填写成定位 
  // else
      // Car_Status[3]&=~0x02;   //Bit(1)    // 河北special 
   ...
   u8  lati_reg[4];//,regstatus;
		  ...
		  if (PositionSD_Status())	
		  {
		         if(DF_LOCK==enable)    // 清除文件区域时 ，禁止操作DF
				 	return ;
			  if((ReadFail_sd_current)&&DataLink_Status())
			  	{      rt_kprintf("\r\n read fail don't save\r\n");   return;}
		     //1.   时间超前判断
                            //  if(Time_FastJudge()==false)    
					//		return ;		 
				  //----------------------- Save GPS --------------------------------------
...
//附加信息7
	 Original_info[Original_info_Wr++]=0x25; //信号强度
	 //  附加信息长度
	 Original_info[Original_info_Wr++]=4; 
	 //  类型
...
void SpeedWarnJudge(void)  //  速度报警判断 
{
					...
					//rt_kprintf("\r\n  超速报緓iaoxiao痋r\n"); 
					 ...
					 if(  JT808Conf_struct.Speed_warn_MAX >0 )   //> 0 
					 {
						 ...
						 //----- GPS  即时速度	0.1 km/h  ---------
							 if  ( GPS_speed>( JT808Conf_struct.Speed_warn_MAX*10) )//(小小)
...
u8 i=0;
      ...
      {           
		...
		{
			/*rt_kprintf("\r\n  %x/%x/%x   %x:%x\r\n",Avrgspd_Mint.datetime_Bak[0],Avrgspd_Mint.datetime_Bak[1],Avrgspd_Mint.datetime_Bak[2],Avrgspd_Mint.datetime_Bak[3],Avrgspd_Mint.datetime_Bak[4]);
			for(i=0;i<60;i++)
				{
				if(i%10==0)
					rt_kprintf("\r\n");
				rt_kprintf("  %2d",Avrgspd_Mint.avgrspd[i]);
				}
			rt_kprintf("\r\n");*/
...
u8   Time_FastJudge(void) 
{
	 ...
	 u32  Date_valueRTC=0,Time_valueRTC=0;	 
	  ...
	  Date_valueRTC=time_now.year*365+time_now.month*60+time_now.day;
         Time_value1=Gps_Gprs.Time[0]*3600+Gps_Gprs.Time[1]*60+Gps_Gprs.Time[2];
